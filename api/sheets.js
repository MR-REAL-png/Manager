...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // GET GROUP TRANSFERS - fix: pakai user_id (case-sensitive match dengan tabel users)
    if (req.method === 'GET' && action === 'get-group-transfers') {
      ...
      // Fix: pakai user_id karena case-nya match dengan tabel users (Sheril = Sheril)
      // input_by lowercase (sheril) tidak match username (Sheril)
      const { data, error } = await supabase
        .from('transfers')
        .select('*')
